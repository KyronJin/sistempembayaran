...
<div class="container mx-auto px-4 py-6">
  <div class="flex items-center justify-between mb-4">
    <h1 class="text-2xl font-bold text-gray-800"><i class="fas fa-cash-register mr-2"></i>Laporan Penjualan Saya</h1>
    <div class="flex gap-2">
      <button onclick="window.print()" class="btn-primary"><i class="fas fa-print mr-2"></i>Cetak Laporan</button>
      <a href="{{ route('kasir.reports.index') }}" class="btn-secondary"><i class="fas fa-arrow-left mr-2"></i>Kembali</a>
    </div>
  </div>

  <form method="GET" action="{{ route('kasir.reports.sales') }}" class="bg-white rounded-lg shadow p-4 mb-4 no-print">
    ...
    <div class="mt-3">
      ...
      <a href="{{ route('kasir.reports.sales') }}" class="btn-secondary ml-2"><i class="fas fa-times mr-2"></i>Reset</a>
    </div>
  </form>
  ...
  <div class="grid md:grid-cols-3 gap-3 mb-4">
    ...
    <div class="bg-white rounded-lg shadow p-4">
      ...
      <div class="text-xl font-bold">Rp {{ $totalTransactions > 0 ? number_format($totalSales/$totalTransactions,0,',','.') : 0 }}</div>
    </div>
  </div>
